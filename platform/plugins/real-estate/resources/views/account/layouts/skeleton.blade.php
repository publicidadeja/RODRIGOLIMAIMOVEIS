...
<body @if ($isRTL) dir="rtl" @endif>
  ...
  <div id="app">
    <div id="admin_panel_url" data-url="{{ BaseHelper::getAdminPrefix() }}" style="display: none;"></div>
    ...
    <main class="pv3 pv4-ns">
      ...
      @if (is_plugin_active('language'))
          ...
          @if ($supportedLocales && count($supportedLocales) > 1)
              @if (count(\Srapid\Base\Supports\Language::getAvailableLocales()) > 1)
                  <footer>
                      <p>{{ __('Languages') }}:
  ...
  <script src="{{ asset('vendor/core/plugins/real-estate/js/account-property.js') }}"></script>
  {!! Assets::renderFooter() !!}
  @stack('scripts')
  @stack('footer')
</body>
